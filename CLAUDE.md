...
Run `update_cache_incremental.py` first (Step 2), then this script. It raises a clear error rather than guessing if the `Weekly Report Manual` sheet has an ambiguous/duplicate `Week_1st_day` (see the 2026-09-16 Open item below for precedent) — fall back to `--manual-cur-idx`/`--manual-prev-idx` (0-based row ranges) after inspecting the sheet by hand.
