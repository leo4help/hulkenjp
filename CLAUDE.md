...
**As of 2026-09-16, use `scripts/build_weekly_report.py`** — it computes every data const below from the cache+Bridge and templates the new HTML from the previous week's report in one run. Before this script existed, every weekly build had to reverse-engineer the whole methodology from scratch by diffing against last week's published HTML (this is what ate ~30 minutes on the W37 build) — don't repeat that; use the script.
...
Run `update_cache_incremental.py` first (Step 2), then this script. It raises a clear error rather than guessing if the `Weekly Report Manual` sheet has an ambiguous/duplicate `Week_1st_day` (see the 2026-09-16 Open item below for precedent) — fall back to `--manual-cur-idx`/`--manual-prev-idx` (0-based row ranges) after inspecting the sheet by hand. **Fixed 2026-09-23 (W38):** the check now only raises when a `(Week_1st_day, Channel, AD Type)` triple repeats — the old check looked at `Channel` alone, which always repeats (Google/Amazon/Rakuten each have 品牌/非品牌 rows), so it false-alarmed every week. `--manual-cur-idx`/`--manual-prev-idx` are iloc slices (end-exclusive).

The script's own module docstring and inline comments carry the full methodology reference (which const sources from which sheet, the CPM×1000 rule, population/delta-baseline rules for `metaTestKol`/`metaCreative`/`kolRows`, `NAME_ALIASES`) — read it before modifying the script, and update it in the same commit if the methodology changes. The condensed version of that same methodology is also kept below in "Data-source & computation reference" so it's discoverable from this file without opening the script.

After running it, still **manually verify**: the output HTML in a browser, that the insight-callout block situation is correct for this week (see below — the script only removes a stale one, it never adds a new one), and that `AD Images/` has thumbnails for any new Rows.

Always start from the most recent week's HTML as the template (the script does this for you) — never rebuild the CSS/JS/layout by hand.
...
- **The insight callout block (added 2026-08-20-ish, re-affirmed as a hard requirement 2026-09-10 after it went missing for two weeks running).** Inside the `#channels` section, immediately after its `<div class="hint">%Δ 為對比上週...</div>` line, there is (or should be) a `<div class="callout">...</div>` block with one `<div class="row">...</div>` per row of the Bridge/Mastersheet's `Weekly Insight` tab where `Section == '全渠道'` and `Week_1st_day` equals this week's own start date — copy the `Note` text verbatim, don't paraphrase or invent. If there are no such rows for this week, omit the whole `<div class="callout">` block (per Mastersheet_Tab_Guide.md — don't fabricate insights). **This step is easy to miss because copying the previous week's HTML as a template silently carries forward whatever that week did** — it went missing starting W35 (dropped with no one noticing) and stayed missing into W36 until the user caught it and had it restored on 2026-09-10. Explicitly check for this block, every week, even though it's easy to overlook since it's plain HTML rather than one of the `const` data blocks above.
  > **Bug found + fixed 2026-09-16: removing this block with a naive regex silently truncates it instead of deleting it.** The callout div nests one `<div class="row">` per insight line, e.g. `<div class="callout"><div class="row">A</div><div class="row">B</div><div class="row">C</div></div>`. A non-greedy `<div class="callout">.*?</div>` regex stops at the FIRST `</div>` it finds — the close tag of row A — so it only deletes the opening tag and the first row, leaving every other row (and the outer closing `</div>`) dangling in the page as unwrapped, visibly-rendered plain text instead of being removed. This happened on the live W37 report (built with no Weekly Insight data on purpose, but 3 leftover rows from W36's own callout still showed up as bare text under the Overview table) and was only caught because the user spotted the stray text and asked why it was there. `scripts/build_weekly_report.py`'s `_remove_balanced_div()` fixes this by counting nested `<div>`/`</div>` tags to find the true matching close tag — don't reintroduce a plain regex for this removal.
...
> **Data-source & computation reference (written up 2026-09-16, after having to re-derive all of this from scratch for the W37 build — kept here so it never has to be rediscovered again; `scripts/build_weekly_report.py` implements exactly this).**
>
> - **Two genuinely different KPI scopes — don't conflate them.** (1) The 6 hero `kpis` tiles are computed straight from `ad_data` (Meta+Google combined, historical cache + current Bridge window, `goal not in ['-','CPC']`) — NOT from the Weekly Report Manual sheet. (2) `channelRows`/`channelGrand` (the Overview·全渠道 table and its grand total) come entirely from the **`Weekly Report Manual`** sheet in the Bridge file instead — that's the only place Amazon/Rakuten numbers exist, since `ad_data` itself never includes those two channels.
> - **CPM is always `cost/impressions*1000`** (JPY per 1000 impressions) — every place CPM is computed, double check the `×1000` is actually there; it's the single easiest thing to silently drop.
> - **`metaTestKol` is NOT KOL-only** despite the name (kept for historical continuity, same pattern as `all.html`'s `kolLifetimeRows`/`computeKolAggregate`) — it's every Meta creative (KOL + 原廠) with `testFlag=='TEST'`.
> - **`kolRows`/`kolGrand` group by `isKOL` only** (never by `testFlag`), and their population is the union of: this week's KOL media NameRows, last week's KOL media NameRows, this week's KOL license-ghost NameRows, and last week's KOL license-ghost NameRows — so a KOL with a license ghost row but zero media spend yet (a signed-but-not-yet-live creative) still shows up. `costD` (and every other delta field) must be `None`, not a computed number, when there was **no previous-week MEDIA presence** for that NameRow — even if a previous license-only presence existed, since a %Δ against "license fee, zero media" is meaningless.
> - **KOL license "ghost" rows** (`goal=='-'` restricted to Meta AD + the KOL NameRow regex) are the ONLY source for `kolRows`/`kolGrand`'s `lic` figures — never the separate `KOL授權金` Bridge tab (that tab exists purely for `all.html`'s negotiation-calculator `firstWave` field; see Step 6).
> - **All other WoW %Δ fields** for `metaTestKol`/`metaCreative`/`kolRows` are computed against the previous week's **unified per-NameRow baseline across all buckets/testFlags** — never a bucket-scoped previous value (a creative can flip between `General` and `TEST` week to week purely from a `TEST | ` name-prefix rename, per the 2026-08-26 note above; the baseline must follow the NameRow, not the bucket).
...
1. Recompute `kolLifetimeRows` by running **`scripts/build_all_lifetime.py`** (added 2026-09-16 — see the note right below; this is the permanent replacement for the never-saved `compute_all_lifetime.py`):
   ```bash
   ...
   ```
   `--bridge-cutoff` should match the historical cache's current cutoff (check `_cache/ad_data_cache_meta.json`) so this week's Bridge rows are appended without double-counting. This one script call also does step 3 below (the chip/date-text updates) — no separate manual edit needed. Methodology (for reference / if the script ever needs modifying): for each `NameRow` in Meta creative with spend, media rows are `goal not in ['-','CPC']` restricted to `AD_Channel=='Meta AD'`, license "ghost" rows are `goal=='-'` (also Meta-only), both over the full combined `ad_data` (historical cache + current Bridge, no window filter). Per row: `mediaCost/purchases/revenue/clicks/impressions` summed from media rows, `lic` summed from ghost rows, `cost=mediaCost+lic`, `roas=revenue/cost`, `mediaRoas=revenue/mediaCost`, `cpa=cost/purchases`, `ctr=clicks/impressions*100`, `cvr=purchases/clicks*100`, `cpm=mediaCost/impressions*1000`, `isKOL=mode(sub.isKOL)`, `tag=mode(sub.原廠標記.dropna())` (or `null` if none). Same formulas as the weekly `kolRows` table, just unwindowed and un-restricted to KOL.
   > **Resolved 2026-09-16 (was an open item since W35):** `compute_all_lifetime.py` was never actually saved to disk in an earlier session — it was written and run once in that session's own scratch space and lost when the session ended, which is why the B/C/`LIC_ROW_DATA` refresh silently stopped happening for a couple of weeks. `scripts/build_all_lifetime.py` is the validated, permanent, git-tracked replacement — this should not recur. If a future session ever again computes something ad hoc without saving the script, save it before the session ends, even if it means asking for one extra turn to do so.
2. Recompute `LIC_ROW_DATA`'s `B`/`C`/`activeDays`/`windowCost`/`windowRev`/`firstActive`/`lastActive`/`usedAllTime` fields with **`scripts/update_lic_row_bc.py`** (added 2026-09-23, W38 — before this it had never been scripted, and every Row's B/C had silently stayed frozen at the W33 build, `lastActive` 2026-08-19). Run it **before** `update_kol_license_calc.py` so Rows it newly adds also get their `firstWave`:
   ```bash
   ...
   ```
   Window (confirmed by the user 2026-09-23, final): **per Row, its most recent 14 days WITH SPEND up to the report week's end date** — not 14 calendar days. The window stretches back per Row as far as needed, so a Row that paused recently still gets B/C from its last 14 active days (e.g. Row 10's are 2026-03-11~03-24) instead of coming up empty. The user tried a 28-day calendar window and then lifetime the same day before settling on this — don't switch without asking. B = media spend over those days ÷ activeDays (日均花費), C = purchase value ÷ media spend (媒體單獨 ROAS); media rows only (Meta AD, goal not in `-`/`CPC`), no license fee. A Row with fewer than 14 spend days in total uses all of them (`usedAllTime: true`, shown as 全期間數據); otherwise the badge reads 「近14個投放日數據」 (the script keeps that label in sync). Default is `--last-spend-days 14`; `--window-days N` / lifetime remain as options only. The calculator's static explanatory text in `all.html` (the `.calc-desc` line under step ① and the B/C `.calc-hint` lines) was rewritten to describe this 14-spend-day rule on 2026-09-23 — the script does NOT regenerate that text, so if the window rule ever changes, update those three strings by hand too.
...
│   ├── update_lic_row_bc.py            ← added 2026-09-23: refreshes all.html's calculator B/C (each Row's last 14 spend days up to week end) — see Step 6, run before update_kol_license_calc.py
│   ├── build_weekly_report.py          ← added 2026-09-16: computes all report data consts + templates the new week's HTML — see Step 3
│   └── build_all_lifetime.py           ← added 2026-09-16: recomputes all.html's kolLifetimeRows + date chips — see Step 6 (permanent replacement for the lost compute_all_lifetime.py)
